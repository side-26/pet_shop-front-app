...
3. Load `api-input-contracts` for request input typing and contract-source selection.
4. Load `entity-layer-architecture`; for forms or UI states also load `shadcn-pet-ui-component`.

## Required layers

- **DTO (`*.dto.ts`)**: Define request and response wire contracts. Derive request input types from their runtime validation schema when one exists; otherwise type them from the authoritative prompt or linked endpoint specification. Preserve backend `TData`, including `null`.
...
2. Derive the API request input from that inferred type instead of repeating validated fields in a handwritten DTO. Use `Pick` or `Omit` when validated UI-only fields are not sent.
3. Pass that schema to the shared `Form`; validate again in the Server Action.
4. Keep form refs and `setError` plumbing in the client layer.
5. Pass form submission state to the shared Button through `isLoading` and its required `loadingText`. Button owns busy behavior and replaces its children; do not render loading Spinner or conditional loading text at the call site.
6. Use Button's `block` prop for full-width form actions instead of passing `tw:w-full` through `className`.
...
1. Confirm validation exists only in the schema layer, its API input type is inferred rather than duplicated, and it is enforced server-side.
