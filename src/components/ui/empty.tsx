...
function Empty({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      data-slot="empty"
      className={cn(
        'tw:flex tw:w-full tw:min-w-0 tw:flex-1 tw:flex-col tw:items-center tw:justify-center tw:gap-4 tw:rounded-2xl tw:border-dashed tw:p-12 tw:text-center tw:text-balance',
        className,
      )}
      {...props}
    />
  ...
  'tw:mb-2 tw:flex tw:shrink-0 tw:items-center tw:justify-center tw:[&_svg]:pointer-events-none tw:[&_svg]:shrink-0',
  {
    variants: {
      variant: {
        ...
        icon: 'tw:flex tw:size-10 tw:shrink-0 tw:items-center tw:justify-center tw:rounded-xl tw:bg-muted tw:text-foreground tw:[&_svg:not([class*=size-])]:size-5',
      },
    },
    defaultVariants: {
      variant: 'default',
...
function EmptyTitle({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('tw:text-title-m tw:tracking-tight', className)}
      {...props}
    />
  );
}
